...
export function ClientsPage() {
  ...
  const fetchClients = async () => {
    ...
    try {
      ...
      const response = await api.get(`/modules/inventory/api/clients?${params.toString()}`);
  ...
  const handleCreate = async () => {
    ...
    try {
      await api.post('/modules/inventory/api/clients', formData);
      notifications.show({
        title: 'Success',
      ...
      notifications.show({
        title: 'Error',
        message: error.response?.data?.detail || 'Failed to create Client',
        color: 'red',
      });
    }
  };
  ...
  const handleDelete = (id: string, name: string) => {
    modals.openConfirmModal({
      ...
      onConfirm: async () => {
        try {
          await api.delete(`/modules/inventory/api/clients/${id}`);
          notifications.show({
            title: 'Success',
  ...
  const getCountry = (Client: Client) => {
    if (Client.addresses && Client.addresses.length > 0) {
      return Client.addresses[0].country || '-';
    }
    return '-';
  };
          ...
          <Table.Tbody>
            {Clients.map((Client) => (
              <Table.Tr key={Client._id}>
                ...
                <Table.Td>{formatDate(Client.created_at)}</Table.Td>
                <Table.Td>
                  <Group gap="xs">
                    <ActionIcon
                      variant="light"
                      color="blue"
                      onClick={() => navigate(`/inventory/Clients/${Client._id}`)}
                    >
                      <IconEdit size={16} />
                    </ActionIcon>
                    <ActionIcon
