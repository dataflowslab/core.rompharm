...
class ApprovalSettings(BaseModel):
    ...
    can_sign: List[Dict[str, str]] = []  # [{'user_id': 'xxx', 'username': 'xxx'}]
    must_sign: List[Dict[str, str]] = []  # [{'user_id': 'xxx', 'username': 'xxx'}]
...
class FormCreate(BaseModel):
    ...
    approval_settings: Optional[ApprovalSettings] = None
...
async def create_form(form_data: FormCreate, request: Request, user = Depends(verify_admin)):
    ...
    if form_data.approval_settings:
        approval_settings = form_data.approval_settings.dict()
...
async def update_form(form_id: str, form_data: FormUpdate, request: Request, user = Depends(verify_admin)):
    ...
    if form_data.registry_start is not None:
        ...
        # Only update registry_current if it's not set yet
        existing_form = forms_collection.find_one({'_id': obj_id})
        if existing_form and not existing_form.get('registry_current'):
            update_doc['registry_current'] = form_data.registry_start
    if form_data.approval_settings is not None:
        update_doc['approval_settings'] = form_data.approval_settings.dict()
    ...
    form = forms_collection.find_one({'_id': obj_id})
    ...
    log_action(
        ...
        details={'updated_fields': list(update_doc.keys()), 'title': form_title}
    )
...
async def delete_form(form_id: str, request: Request, user = Depends(verify_admin)):
    ...
    # Get form title before deleting
    form = forms_collection.find_one({'_id': obj_id})
    form_title = form.get('title', 'Unknown Form') if form else 'Unknown Form'
...
@router.get("/registries/list")
async def list_registries(user = Depends(verify_token)):
    """
    List all forms that have registries enabled
    """
    from datetime import datetime
    db = get_db()
    forms_collection = db[FormModel.collection_name]
    
    # Find all forms with has_registry=True that are not deleted
    registries = list(forms_collection.find({
        'has_registry': True,
        'active': True,
        '$or': [
            {'deleted': None},
    ...
    return [FormModel.to_dict(form) for form in registries]
