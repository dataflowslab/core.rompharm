...
import { returnsApi, Attachment } from '../../services/returns';
...
export function AttachmentsTab({ orderId, attachments, onReload, canEdit }: AttachmentsTabProps) {
  ...
  const handleFileUpload = async (files: File[]) => {
    ...
    try {
      ...
      await api.post(returnsApi.uploadAttachment(orderId), formData, {
        headers: {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await api.delete(returnsApi.deleteAttachment(orderId, attachmentToDelete._id));
      notifications.show({
        title: t('Success'),
