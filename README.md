...
Configuration:
  config.yaml          # Main config (MongoDB, InvenTree, email, etc.)
...
- **Email not sending**: Check `config.yaml` email settings, run `invoke test-email`
...
**No emails received**:
1. Check `config.yaml` has valid Newsman credentials
...
See `config_sample.yaml` for all available configuration options.
