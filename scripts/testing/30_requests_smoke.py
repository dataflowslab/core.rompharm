...
def main() -> int:
    ...
    print(f"[REQ] /modules/requests/api -> {status} count={len(data.get('results', []))}")

    status, data, text = client.request("GET", "/modules/requests/api/states")
    print(f"[REQ] /modules/requests/api/states -> {status} count={len(data.get('results', []))}")
