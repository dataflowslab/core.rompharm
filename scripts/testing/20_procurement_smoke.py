...
def main() -> int:
    ...
    status, data, text = client.request(
        "GET",
        "/modules/depo_procurement/api/purchase-orders",
        params={"limit": 5},
    )
    print(f"[PROC] /purchase-orders -> {status} count={len(data.get('results', data or []))}")

    status, data, text = client.request("GET", "/modules/depo_procurement/api/order-statuses")
    statuses = data.get("statuses", data or [])
    print(f"[PROC] /order-statuses -> {status} count={len(statuses)}")
