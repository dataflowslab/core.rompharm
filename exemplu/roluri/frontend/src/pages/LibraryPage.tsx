...
export function LibraryPage() {
  ...
  const [shareOptions, setShareOptions] = useState<Array<{ value: string; label: string; group?: string }>>([]);
  ...
  useEffect(() => {
    const loadShareOptions = async () => {
      ...
      try {
        const usersPromise = api.get('/api/users/select');
        const rolesPromise = isAdmin ? api.get('/api/users/roles') : Promise.resolve({ data: [] });

        const [usersResult, rolesResult] = await Promise.allSettled([usersPromise, rolesPromise]);

        const userOptions: Array<{ value: string; label: string; group?: string }> = [];
        if (usersResult.status === 'fulfilled') {
          const users = Array.isArray(usersResult.value.data) ? usersResult.value.data : [];
          for (const user of users) {
            const username = user.username || '';
            if (!username) continue;
            const email = user.email && user.email !== username ? ` (${user.email})` : '';
            userOptions.push({ value: username, label: `${username}${email}`, group: 'Utilizatori' });
          }
        }

        const roleOptions: Array<{ value: string; label: string; group?: string }> = [];
        if (rolesResult.status === 'fulfilled') {
          const roles = Array.isArray(rolesResult.value.data) ? rolesResult.value.data : [];
          for (const role of roles) {
            const name = role.name || '';
            if (!name) continue;
            roleOptions.push({ value: name, label: name, group: 'Roluri' });
          }
        }

        setShareOptions([...userOptions, ...roleOptions]);
      } catch (error) {
        ...
        <Stack>
          ...
          <MultiSelect
            ...
            data={[
              ...shareOptions,
              ...editSharedWith
                .filter((value) => !shareOptions.find((option) => option.value === value))
