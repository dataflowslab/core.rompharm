...
import { ApprovalSettings } from '../components/Forms/ApprovalSettings';
...
export function FormsPage() {
  ...
  useEffect(() => {
    console.log('DashboardPage mounted, isAdmin:', isAdmin);
    if (isAdmin) {
      loadForms();
    } else {
      setLoading(false);
  ...
  const viewFormData = (formId: string) => {
    navigate(`/submissions?form=${formId}`);
  };
  ...
  return (
    <Container size="xl" mt={isMobile ? 10 : 30} px={isMobile ? 'xs' : 'md'}>
      ...
      {/* Removed JSON Schema Modal - now in tabs */}
      {jsonModalOpened && (
