...
def _resolve_user_role(db, username: str) -> tuple[str, str]:
    if not username:
        return '', ''
...
async def get_shared_files(
    ...
    # Get user's role (id + name)
    user_role_id, user_role_name = _resolve_user_role(db, user.get('username', ''))
    ...
    if user_role_id:
        or_conditions.append({'shared_with': user_role_id})
    if user_role_name:
        or_conditions.append({'shared_with': user_role_name})
    base_query = {
        '$or': or_conditions,
...
async def get_file_details(file_id: str, user = Depends(verify_token)):
    ...
    username = user['username']
    user_role_id, user_role_name = _resolve_user_role(db, username)
    
    is_owner = file_doc['owner'] == username
    is_shared = (
        username in file_doc.get('shared_with', []) or
        (user_role_id and user_role_id in file_doc.get('shared_with', [])) or
        (user_role_name and user_role_name in file_doc.get('shared_with', [])) or
