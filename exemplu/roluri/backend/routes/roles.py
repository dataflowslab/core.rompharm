...
async def get_available_sections(current_user = Depends(verify_admin)) -> Dict[str, Any]:
    """
    Get all available sections and their permissions
    """
    db = get_db()
    
    try:
        # Get sections structure from config
        sections_config = db.config.find_one({'slug': 'sections_structure'})
        
        if sections_config and 'value' in sections_config:
            return sections_config['value']
...
async def update_role_permissions(
    ...
    """
    Update role permissions (menu_items and sections)
    """
    db = get_db()
    roles_collection = db['roles']
    ...
    # Build update document
    update_doc = {}
    if permissions_data.menu_items is not None:
        update_doc['menu_items'] = permissions_data.menu_items
    
    if permissions_data.sections is not None:
        update_doc['sections'] = permissions_data.sections
