...
class FormModel:
    ...
    def create(slug: str, title: str, json_schema: Dict[Any, Any], 
        ...
        Args:
            ...
            has_registry: Whether form has a registry
            registry_start: Starting number for registry (if has_registry is True)
            approval_settings: Approval workflow settings
        ...
        return {
            ...
            'registry_start': registry_start if has_registry else None,
            'registry_current': registry_start if has_registry else None,
            'approval_settings': approval_settings or {
                'enabled': False,
                'min_signatures': 1,
                'can_sign': [],  # [{'user_id': 'xxx', 'username': 'xxx'}]
                'must_sign': []  # [{'user_id': 'xxx', 'username': 'xxx'}]
            },
